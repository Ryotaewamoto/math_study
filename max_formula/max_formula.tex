...
\begin{itembox}[l]{\underline{Proposition 3.1.1 (Sublinearity) }}
  A function $f$ : $\mathbb{E}$ $\to$ $ \left ( -\infty ,+\infty \right ] $ is sublinear if and only if it is positively homogeneous and subadditive. For a
...
\begin{proof}
  ...
  ( $\mathrm{i}$ ) We prove that $\forall x, y \in lin f$,
  ...
  For the result of ( $\mathrm{i}$ ) and ( $\mathrm{ii}$ ), $lin f$ is a subspace of $\mathbb{E}$.

  ( $\mathrm{II}$ ) We show that for a sublinear function $f$ it holds that

  \begin{center}
    $\forall V \subset \mathbb{E}$, where $V$ is subspace of $\mathbb{E}$, $V \subset lin f$ \\
    $\Leftrightarrow$ $\exists V_0 \subset \mathbb{E}$, where $V$ is subspace of $\mathbb{E}$, $V_0 \not\subset lin f$.
  \end{center}

  Suppose to the contrary that there exists a subspace $V_0$ which is not included in $lin f$,

  \begin{equation}
    \exists x_0 \in V_0 \:s.t.\: x_0 \not\in lin f.
  \end{equation}

  By (1) and the definition of linearity space,

  \begin{center}
    $-f(x_0) \not= f(-x_0)$.
  \end{center}

  However $V_0$ is a subspace of $\mathbb{E}$ so it holds that $-f(x_0) = f(-x_0)$.

  We can find a contradiction to the assumption that $V_0$ is a subspace of $\mathbb{E}$, therefore $lin f$ is the largest subspace of $\mathbb{E}$.
\end{proof}
