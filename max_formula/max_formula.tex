...
\begin{proof} We show these relations.
  ...
  ( $\mathrm{i}$ ) If $\lambda \geq 0$,

  \begin{equation}
    \begin{split}
      q(\lambda \bar{x}) = p'(\bar{x};\lambda \bar{x}) &= \lim_{t \downarrow \infty} \frac{p(\bar{x}+\lambda \bar{x}t)-p(\bar{x})}{t} \\
      &= \lim_{t \downarrow \infty} \frac{p((1 + \lambda t)\bar{x}) - p(\bar{x})}{t} \\
      &= \lim_{t \downarrow \infty} \frac{(1 + \lambda t)p(\bar{x}) - p(\bar{x})}{t} \because \text{$f$ is sublinear}\\
      &= \lambda p(\bar{x}). \notag
    \end{split}
  \end{equation}

  I cannot prove the case of $\lambda<0$.

  ( $\mathrm{ii}$ )$\forall x \in \mathbb{E}$,

  \begin{equation}
    \begin{split}
      q(x) = p'(\bar{x};x) &= \lim_{t \downarrow \infty} \frac{p(\bar{x}+tx)-p(\bar{x})}{t} \\
      &\leq \lim_{t \downarrow \infty} \frac{p(\bar{x}) + tp(x) - p(\bar{x})}{t} \because \text{$f$ is sublinear}\\
      &= p(x). \notag
    \end{split}
  \end{equation}
